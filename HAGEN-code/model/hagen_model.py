    ...
    def forward(self, inputs, adj_mx, nodevec1, nodevec2, hidden_state=None):
        ...
        if hidden_state is None:
            hidden_state = torch.zeros(
                (self.num_rnn_layers, batch_size, self.hidden_state_size), device=device)
        hidden_states = []
        output = inputs
        for layer_num, dcgru_layer in enumerate(self.dcgru_layers):
            next_hidden_state, dw = dcgru_layer(
...
class DecoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, **model_kwargs):
        ...
        self.projection_layer = nn.Linear(
            (self.rnn_units+self.dim_poi), self.output_dim).to(device)
        self.dcgru_layers = nn.ModuleList(
            [DCGRUCell(self.rnn_units, self.max_diffusion_step, self.num_nodes,
                       filter_type=self.filter_type) for _ in range(self.num_rnn_layers)])
...
class graph_constructor(nn.Module):
    def __init__(self, nnodes, k, dim, device, alpha=3, static_feat=None, emb_dir='./embedding_chi.npy'):
        ...
        if static_feat is not None:
            ...
            self.lin1 = nn.Linear(xd, dim, device=device)
            self.lin2 = nn.Linear(xd, dim, device=device)
        else:
            ...
            self.lin1 = nn.Linear(dim, dim, device=device)
            self.lin2 = nn.Linear(dim, dim, device=device)
            self.lin3 = nn.Linear(self.nnodes, self.nnodes, device=device)
        self.device = device
        self.k = k
        self.dim = dim
        self.alpha = alpha
    ...
    def forward(self, idx):
        ...
        # print(nodevec1.device)
        # print(self.lin1.device)
        # nose = self.lin1(nodevec1)
...
class HAGENModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, logger, **model_kwargs):
        ...
        self.gc = graph_constructor(self.num_nodes, subgraph_size, node_dim,
                                    device, alpha=tanhalpha, static_feat=static_feat, emb_dir=emb_dirr)
        ...
        self.emb_crime = nn.Embedding.from_pretrained(
            embedding_crime).to(device)
        self.projection = torch.nn.Linear(
            self.num_nodes, self.num_nodes, device=device)
    ...
    def decoder(self, encoder_hidden_state, adj_mx, nodevec1, nodevec2, POI_feat, labels=None, batches_seen=None):
        ...
        go_symbol = torch.zeros(
            (batch_size, self.num_nodes * self.decoder_model.output_dim), device=device)
        decoder_hidden_state = encoder_hidden_state
        decoder_input = go_symbol
        outputs = []
        for t in range(self.decoder_model.horizon):
            decoder_output, decoder_hidden_state = self.decoder_model(
                decoder_input, adj_mx, nodevec1, nodevec2, POI_feat, labels, decoder_hidden_state)
